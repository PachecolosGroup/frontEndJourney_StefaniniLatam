# 🚀 Mi FrontEnd Code Journey 2025-2026

Este repositorio documenta mi trabajo diario, proyectos y aprendizaje como **Frontend Developer** durante el año 2025-2026.
Decidí subirlo oficialmente desde el 21 de Agosto del 2026, debi hacerlo desde el dia uno, pero tenia demasiadas cosas en la cabeza, muchas cosas por estudiar y organizar. Anywho aca esta y seguire manteniendolo activo.

## 📊 Resumen del año + Tecnologías utilizadas

- **Total de proyectos**: +300
- **Tecnologías principales**: HTML5 semántico, React, HTML, CSS, JavaScript, React, TypeScript, Tailwind CSS, Vite, JavaScript vanilla (ES6+)
- **Horas de trabajo**: 1,264 horas efectivas
- **Estilos**: Tailwind CSS, CSS3 (Flexbox, Grid, animaciones)
- **Build Tools**: Vite, Webpack
- **Control de versiones**: Asure Storage explorer, Git, GitHub
- **Deploy**: Netlify, Vercel, Wordpress

| Tecnología     | % aprox. | Descripción                          |
| -------------- | -------- | ------------------------------------ |
| **HTML5**      | 55%      | Estructura de páginas y componentes  |
| **CSS3**       | 25%      | Estilos, animaciones y responsividad |
| **JavaScript** | 25%      | Interactividad y lógica frontend     |

## 📌 Tipos de tareas incluidas

- Landing pages completas
- Banners promocionales
- Parrillas de productos y catálogos
- Blogs
- Rediseños
- Componentes UI reutilizables (botones, modales, sliders)
- Maquetación de secciones para WordPress
- Optimización de rendimiento frontend

## 🔄 Flujo de trabajo

1. Desarrollo en **VS Code**
2. Subida a **WordPress** (mediante FTP o plugin)
3. Validación en entorno de pruebas
4. Pase a producción con ayuda del equipo

## 🎯 Objetivos del repositorio

- 📚 Documentar mi evolución como desarrollador
- 🏆 Mostrar mi portafolio de trabajo
- 🔍 Facilitar la búsqueda de soluciones pasadas
- 🤝 Compartir conocimientos con la comunidad

## 🌟 Proyectos destacados

1. **[Libreria_FrontEnd]** - [Lidere el desarrollo de una librería de Confianza, donde el objetivo es crear
   una colección de componentes que impulsa la web de Movistar.pe
   Creada con pasión por Stefanini para hacer tu trabajo más fácil.]
   - Tecnologías: [React, css, JavaScript]
   - Enlace: [URL si está deployado]

## 📫 Contacto

- **Portafolio**: [pachecolosgroup.com](https://pachecolosgroup.webflow.io/)
- **LinkedIn**: [URL de LinkedIn](https://www.linkedin.com/in/pachecolos-group/)
- **Instagram**: [@pachecolos_group](https://www.instagram.com/pachecolos_group/)

---

📝 **Última actualización**: Agosto 21 del 2026
